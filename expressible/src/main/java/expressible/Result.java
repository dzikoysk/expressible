...
/**
 * {@link Result} represents value or associated error that caused the absence of the expected value.
 * By definition, Result has to contain one non-null value - the value or the error.
 * If you want to use nullable value or nullable error, you have to use wrapper like {@link Option} to explicitly declare it.
 *
...
public class Result<VALUE, ERROR>  {
    ...
    public static <ERROR> @NotNull Result<Blank, ERROR> ok() {
        return new Result<>(OK, Blank.BLANK, null);
    }
    ...
    public static <VALUE> @NotNull Result<VALUE, Blank> error() {
        return new Result<>(ERROR, null, Blank.BLANK);
    }
